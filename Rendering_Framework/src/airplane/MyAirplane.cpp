...
void MyAirplane::Init(INANOA::MyCameraManager* mcm) {
	...
	// load magic model
	string path = "assets/airplane.obj";
	...
	for (unsigned int v = 0; v < mesh->mNumVertices; ++v)
	{
		...
		databuffer[v * 9 + 8] = mesh->mTextureCoords[0][v][2];
	}
	...
	loadTexture("assets/Airplane_smooth_DefaultMaterial_BaseMap.jpg", this->albedo_handle);
	...
	this->m_dynamicSO->setModelMat(glm::translate(glm::vec3(40, 40, 40)));
	this->m_dynamicSO->setPixelFunctionId(2);

	this->MCM = mcm;
}

void MyAirplane::loadTexture(const string& path, GLuint& handle) {
...
void MyAirplane::updateDataBuffer() {
	this->m_dynamicSO->updateDataBuffer(0, this->m_totalDataByte);
}

DynamicSceneObject* MyAirplane::sceneObject() const {
...
void MyAirplane::updateState(const glm::mat4& viewMat, const glm::vec3& viewPos) {
	...
	//this->getModelMatFromCameraManager(this->MCM);
	//this->m_dynamicSO->setModelMat(this->modelMat);
}
...
void MyAirplane::getModelMatFromCameraManager(INANOA::MyCameraManager* mcm) {
	this->modelMat = mcm->airplaneModelMatrix();
}
