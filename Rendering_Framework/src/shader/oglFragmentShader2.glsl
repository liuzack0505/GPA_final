...
layout (binding = 1) uniform sampler2D original_map;
layout (binding = 2) uniform sampler2D position_map;
layout (binding = 3) uniform sampler2D normal_map;
layout (binding = 4) uniform sampler2D diffuse_map;
layout (binding = 5) uniform sampler2D ambient_map;
...
void main() 
{
	...
	vec3 gOriginal = texelFetch(original_map, ivec2(gl_FragCoord.xy), 0).rgb;
